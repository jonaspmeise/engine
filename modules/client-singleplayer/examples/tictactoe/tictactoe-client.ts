...
export class TicTacToeClient extends Client<HTMLDivElement, TicTacToeMark> {
  ...
  render(
    ...
    const horizontalLanes = entityHandler.entities(HorizontalLane);
    for (const lane of horizontalLanes) {
      ...
      const wonBy = lane.wonBy(entityHandler);
      if (wonBy) {
        laneElement.classList.add(`player-${wonBy.mark}`);
      }

      // Render slots.
      const slots = lane.slots(entityHandler);
